...
function App() {
  ...
  const [{sorted, possibleAnswers}, setWordleState] = useState({ answers: [], occurrences: {}, rankings: {}, sorted: [], possibleAnswers: []})
  ...
  const setAnswers = (answers) => {
    ...
    try {
      ...
      const sorted = [...answers].sort((a, b) => {
        console.log(rankings[b], rankings[a]);
  ...
  return (
    <div style={{display:'flex', flexDirection: 'column', alignItems: 'center'}} >
      ...
      <div><Omit value={omit} onChange={setOmit}/></div>
        ...
        <Words onSubmit={setAnswers} disabled={loading} />
    </div>
  );
}
